...
class SalesCockpitApiTests(TestCase):
    ...
    @patch("management.bot_views.bot.notify_manager")
    def test_transfer_to_manager_is_idempotent_and_stops_automation(self, notify_manager):
        from management.models import IgFollowUpTask
        ...
        self.assertFalse(second.json()["transferred"])
        self.active.refresh_from_db()
        self.assertTrue(self.active.bot_paused)
        self.assertTrue(self.active.manager_takeover)
        self.assertEqual(self.active.paused_reason, "manager_takeover")
        self.assertEqual(self.active.stage, IgClient.Stage.LEAD_TO_MANAGER)
        self.assertFalse(
            IgFollowUpTask.objects.filter(
                client=self.active, status=IgFollowUpTask.Status.PENDING
            ).exists()
        )
        self.assertEqual(self.active.stage_events.filter(reason="manual_transfer").count(), 1)
        notify_manager.assert_called_once()

    def test_bot_page_exposes_ukrainian_manager_transfer_action(self):
        html = self.client.get(reverse("management_bot")).content.decode("utf-8")

        self.assertIn("Передати менеджеру", html)
        self.assertIn('data-act="transfer"', html)
