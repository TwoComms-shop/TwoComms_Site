...
Нормализация действует **с момента деплоя** (новые сессии). Исторические ChatGPT-алиасы нормализуются dry-run-first командой `normalize_ai_attribution` только после приватного бэкапа и с точными `--expect-*` guard-параметрами. Остальные исторические источники требуют отдельной инвентаризации и не должны переименовываться этой AI-командой (RISK-07).
