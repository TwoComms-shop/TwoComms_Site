...
Нормализация действует **с момента деплоя** (новые сессии). Существующие строки `UTMSession` с грязными значениями при необходимости бэкфиллятся отдельным management-скриптом (только после бэкапа БД — RISK-07).
