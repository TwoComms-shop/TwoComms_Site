...
def serialize_transaction(txn: Transaction, *, running_balance=None) -> dict:
    ...
    return {
        ...
        'account_type': txn.account.type if txn.account else '',
        'account_is_business': bool(txn.account.is_business) if txn.account else False,
        'account_label': txn.account.name if txn.account else '',
