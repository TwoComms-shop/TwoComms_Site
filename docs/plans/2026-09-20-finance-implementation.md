...
- [x] `USER` При подтверждении «Так, це мої готівкові» превращать импортированное зачисление в единый внутренний перевод `Готівка -> рахунок зарахування` на дату банковской операции. Уменьшать наличные, не создавать искусственный доход и оставлять банковские реквизиты/комментарий неизменными.
- [x] `DERIVED` Для терминального перевода показывать только активные счета типа «Готівка», блокировать подтверждение при недостаточном остатке и классифицировать личную карту как `owner_draw`/`personal`.
...
- [x] `USER` «Людина в біді»: сохранить 216 000 грн на белой monobank-карте отдельным грантовым источником; до расходов показывать как доступное целевое финансирование, не как выручку.
- [x] `USER` В карточке неизвестного входящего надходження дать выбор конкретной грантовой программы, включая «Людина в біді», перед подтверждением `grant_inflow`.
- [x] `USER` В модалке входящей операции добавить явный переключатель «Цільове грантове надходження» с выбором программы, визуальным бейджем в журнале и записью в ledger.
- [x] `DERIVED` В Cash Flow исключить подтверждённые гранты из операционного графика и показать их отдельным KPI «Цільові гранти».
- [x] `USER` Разрешить пометить грантовой любую доходную или расходную операцию с выбором конкретной программы в основной и дополнительной секции модалки.
- [x] `USER` Исторические фактические операции счёта «Грантова» подтвердить как операции УВФ и распределить расходы по источнику; будущие операции оставлять для обязательного review.
- [x] `DERIVED` Новые фактические операции на счёте «Грантова» автоматически получают review-предложение без изменения экономического смысла до подтверждения.
- [x] `USER` На вкладке «Гранти» показывать историю подтверждённых поступлений, очередь неизвестных входящих и действия для подтверждения/распределения расходов.
...
### Текущий срез: пенсія, ФОП-надходження та комісія переказу

- [x] `USER` Додати окремий economic kind `pension_income`; пенсійний рахунок має швидку дію «Так, це пенсія», а інші надходження залишаються ручним вибором.
- [x] `USER` Для бізнесового ФОП-рахунку додати швидку дію «Так, це продаж»; на особистих картках ця дія недоступна.
- [x] `USER` Зіставляти особисте зарахування з наявним списанням без ручного ID та показувати різницю як комісію банку.
- [x] `USER` В модалці будь-якої фактичної операції дати український сценарій «Об’єднати в переказ»: вибір наявної зустрічної операції з рахунком/періодом, або створення відсутньої частини, попередній маршрут і комісія з явним підтвердженням.
- [x] `DERIVED` Зберігати комісію в `InternalTransferMatch`, класифікувати її як `transfer_fee` і включати до P&L/Cash Flow один раз.
- [ ] `USER` Додати окреме редагування банківської комісії та категорії у drawer операції після підтвердження.
...
- [x] `DERIVED` Возвраты аренды/коммунальных от Віктора Викторовича с устойчивым правилом классифицируются автоматически как `expense_refund`; импортированные terminal/City24 операции с похожими реквизитами исключаются.
