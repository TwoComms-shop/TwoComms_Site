...
- [ ] **P0.A10 Add task-based Gemini reasoning routing.** Replace the global `low`/legacy `thinkingBudget=0` behavior with the versioned task matrix in §3.4: customer chat is at least `medium`; product/size/media/payment/order/conversion decisions use `high`; probes remain `low`. Convert tasks to a tested Gemini 2.5 fallback budget without ever sending both controls. For Gemini 3.x remove explicit `temperature`/`topP`/`topK` so the provider's reasoning-optimized defaults remain effective; preserve compatible sampling on 2.5 fallbacks. Return and persist bounded task/level/policy/token telemetry, never thought text. Acceptance: red/green tests for 3.6 chat, every high-stakes task class, 2.5 fallback, low probe, invalid task/level rejection, thought-part filtering, sampling normalization, and all six production keys on the effective 3.6 model.
- [ ] **P0.A11 Make the selected chat model authoritative for pooled keys.** `_run_with_pool()` applies `model_override` to its manual-key model list, but `gemini_keys.iter_attempts(role)` rebuilds the default chain and ignores that override for the six-key pool. Pass one validated chain into both paths, test non-default allowed primary selection and fallback ordering, and expose configured versus actual model without claiming authority before the pooled path proves it.
- [ ] **P1.A11 Persist versioned customer-intelligence snapshots.** Add append-only snapshots with score band, bounded purchase estimate, independent confidence, positive/negative evidence, uncertainties, last analyzed message, model/prompt/rules version, latency, and trigger. Preserve current `buying_readiness` only as a labeled legacy compatibility field until all consumers migrate.
- [ ] **P1.A12 Build an explicit interaction taxonomy.** Classify `reaction_only`, `information_only`, `product_interest`, `size_fit_question`, `custom_print`, `price_objection`, `high_intent`, `payment_pending`, `paid_order_waiting`, `no_reply`, `explicit_no_buy`, `opt_out`, `spam_abuse`, and `unknown`. Story reactions/emoji-only messages are stored and measured but do not automatically receive a sales reply unless an editable business rule explicitly enables it.
