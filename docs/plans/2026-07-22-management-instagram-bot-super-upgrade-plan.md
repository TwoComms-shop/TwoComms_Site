...
- [ ] **P0.2 Restore daemon liveness.** Harden `run_instagram_bot --ensure`, install guarded cron, fix absolute paths/locks, and expose honest offline/dead-worker states. Test concurrent ensure and heartbeat ages. Commit/push/deploy.
- [ ] **P0.3 Deduplicate takeover Telegram alerts.** Add transition/epoch logic and regression tests for repeated manager echoes, resume, and concurrent events. Commit/push/deploy.
- [ ] **P0.4 Add idempotent notification outbox.** Route permanent delivery, AI failure, spam, payment-link failure, takeover, and recovery alerts through one deduplicated mechanism. Verify Telegram call count and persisted status. Commit/push/deploy.
- [ ] **P0.5 Make Gemini 3.6 authoritative.** Add `gemini-3.6-flash` to model/key policy, make the settings model effective, and add model-aware generation config/finish-reason handling. Run mocked tests plus the six-key read-only production probe. Commit/push/deploy.
- [ ] **P0.6 Make key rotation health-aware.** Add parallel two-key preflight, all-key fallback, cooldown persistence, key/model telemetry, and no-secret UI/logging. Commit/push/deploy.
- [ ] **P0.7 Fail closed on webhook signature in production.** Add explicit dev override, configuration health warning, and webhook signature/event-variant tests. Commit/push/deploy.
- [ ] **P0.8 Guarantee no duplicate customer send.** Add response idempotency/attempt state, test webhook retry + poll + stale worker + post-send exception races, and verify no duplicate `InstagramBotMessage` model rows. Commit/push/deploy.
