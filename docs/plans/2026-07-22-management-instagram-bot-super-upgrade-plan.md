...
- [ ] **P1.A16 PARTIAL — replace cumulative readiness inflation with reversible scoring.** Repeated signals are now idempotent per source message, later neutral/deferred evidence can decay or cap readiness, explicit refusal can move it to zero, pure communication opt-out preserves commercial state, and verified provider-ledger payment resolves it to `100`. The policy version advanced to `2026-07-24.v3`; probability/confidence remain separate in snapshots. Focused production-style DB-free tests pass 8/8, including Ukrainian/Russian deferral phrases, manager/reaction preservation, and payment precedence. Remaining acceptance work: 100+ representative Ukrainian/Russian fixtures covering terse speech, sarcasm, mixed language, reactions, abuse, custom print, stock/size ambiguity, promise-to-pay, verified payment, refund, and repeat purchase, plus MariaDB rollback-only persistence/concurrency proof and calibration against verified outcomes.
...
- [ ] **P0.B5 Separate observation/analysis from global and per-client reply enablement.**
  ...
  - [ ] **P1.A16a Keep communication opt-out separate from reversible commercial scoring.**
    ...
    - **Implementation/evidence:** `_resolve_readiness()` now preserves pure opt-out while keeping `no_buy` hard-zero and verified-ledger payment authoritative. Focused production-style DB-free suite passes 8/8 with the unused database skipped; no transport was called.
