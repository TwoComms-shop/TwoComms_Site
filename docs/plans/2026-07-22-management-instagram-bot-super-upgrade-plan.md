...
- [ ] **P0.A10 Add task-based Gemini reasoning routing.** Replace the global `low`/legacy `thinkingBudget=0` behavior with the versioned task matrix in §3.4: customer chat is at least `medium`; product/size/media/payment/order/conversion decisions use `high`; probes remain `low`. Convert tasks to a tested Gemini 2.5 fallback budget without ever sending both controls. Return and persist bounded task/level/policy/token telemetry, never thought text. Acceptance: red/green tests for 3.6 chat, every high-stakes task class, 2.5 fallback, low probe, invalid task/level rejection, and all six production keys on the effective 3.6 model.
