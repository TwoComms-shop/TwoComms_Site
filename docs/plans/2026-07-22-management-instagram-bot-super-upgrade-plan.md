...
- [ ] **P0.A12 Harden per-conversation message pagination.** `poll_ingest()` still trusts cached conversation IDs and nested `messages.paging.next` shapes independently of the already-hardened `/conversations` discovery path. A malformed `messages`/`paging` value can raise before the next conversation, non-string message IDs are silently coerced, an untrusted or repeated `next` URL can be requested, and five sequential 30-second page timeouts per conversation can starve the daemon heartbeat. Validate the cache boundary and every response envelope, require bounded string IDs, allow only centrally versioned `graph.facebook.com/v25.0` next URLs, detect page cycles, preserve cursor/snapshot truth on partial failure, and enforce a per-poll time/request budget. Add malformed-shape, hostile-host, repeated-page, stale-cache, partial-page, and bounded-latency tests before enabling polling as a recovery path.
