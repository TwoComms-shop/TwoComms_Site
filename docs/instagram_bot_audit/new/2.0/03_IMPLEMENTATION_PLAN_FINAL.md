...
**Срез 1: готовится выпуск visibility/fairness; ещё не выпущен.** Базовый HEAD `21eb25986`; staged D071/0209 остаётся отдельным незавершённым пакетом. Проверки запускаются shared CPython3.14.6/Django6.1 с `--settings=test_settings` на локальной временной БД.
...
**Следующее действие root:** scoped commit/push/SSH pull только visibility/fairness + plan; сверить exact SHA, supervisor, MariaDB read-model для171 и прогресс analysis. Затем вернуть whitelist `source_preference_fallback` и завершить вторую группу: preference fallback как отдельный deterministic receipt (без подделки provider winner), canonical presence и исправленный intent/send boundary. READ_ONLY Astra review выявил глобальные case blockers, stale client и gap между final intent check и реальным send; исправления выполняет тот же reviewer в явном EDIT_SCOPED. Presence producer уже удалил legacy blocking pulse; 75 тестов, реальная account visibility/refresh TTL остаются отдельным gate. B03.17/19 и UI-материальность — следующая волна.
