...
**Выполнение: 29 из 196 пунктов · взвешенный прогресс 23.4% · в работе 20 · блокировано 0 · не закрыто 167.** Блоков: 1/17 полностью закрыто, 16 остаются. Из закрытых пунктов один — исторический локальный B02.1; подтверждение новых production выпусков находится непосредственно у пунктов.
...
| B03 | P0 | Turn, outbox и ручное управление | 11% | 8/21 | ~ В работе |
| B04 | P0/P1 | Бесплатные модели и очередь | 7% | 1/12 | ◐ Частично |
| B05 | P1 | Память, понимание и материальность | 11% | 0/11 | Не начат |
...
**Следующее действие root:** whitelist `source_preference_fallback` возвращён локально; завершить вторую группу: preference fallback как отдельный deterministic receipt (без подделки provider winner), canonical presence и исправленный intent/send boundary. READ_ONLY Astra review выявил глобальные case blockers, stale client и gap между final intent check и реальным send; исправления выполняет тот же reviewer в явном EDIT_SCOPED. Presence producer уже удалил legacy blocking pulse; 75 тестов, реальная account visibility/refresh TTL остаются отдельным gate. B03.17/19 и UI-материальность — следующая волна. Дополнительный review обнаружил сохранение старого fit после явного отказа: Astra/high реализует withdrawal в существующем reducer/provenance. Root добавил USER watermark gate после обработки inbox: корректировка не должна обходить stale-reply fence. Новый live intent gate проходит интеграцию; выявлены недостаточные source fixtures и пропущенное явное «Оформлюйте замовлення», исправления проверяются до выпуска. Первый общий запуск второго пакета116tests:112passed/4failed, не считать зелёной приёмкой. Новый fallbackintegration4tests уже passed, расширение negative corpus в работе.
...
**Следующее действие:** scoped commit/push/штатный SSH pull D073.2, проверить SHA/check/supervisor и read-only текущие receipts. Затем B03.17/19/21 и C18 materiality/graph. Старые клиенты аудита не получают тестовые сообщения/автоматический replay; незавершённые cases сохраняются до доказанной coverage.
...
- [~] **B03.16 — P0: предпочтение без товара и полезный ответ после отказа валидатора.**
