...
- [x] **CRO-041. КРИТИЧНО: COD-заказ не пишет UTM.** ПРОВЕРЕНО 05.07.2026 (код + БД) — подтверждён, детальный разбор и ТЗ: `audit_report_section1_cro.md`. Где: checkout.py — в нём НЕТ вызовов `link_order_to_utm`/`record_purchase`/`record_lead` (они есть только в `storefront/views/monobank.py:565,584,986`). Подтверждено БД: 0 из 41 заказов имеют utm_source. Что: задача = вызывать `link_order_to_utm(request, order)` + `record_order_action` при создании ЛЮБОГО заказа (COD и предоплата). Это TECH-060.
- [x] **CRO-042. is_converted никогда не проставляется.** ПРОВЕРЕНО 05.07.2026 — подтверждён (0/1015 converted; онлайн-оплат ещё не было → путь record_purchase ни разу не исполнялся), разбор: `audit_report_section1_cro.md`. Где: `storefront/utm_tracking.py::record_lead/record_purchase` вызываются только из monobank-потока; БД: 1015 UTM-сессий, 0 converted. Что: после фикса CRO-041 проверить, что `mark_as_converted` срабатывает и для COD. Это TECH-061.
...
- [x] **AN-013. fbc/fbp/fbclid доходят до CAPI.** ПРОВЕРЕНО 05.07.2026 — подтверждён: CAPI берёт fbc/fbp только из `order.utm_session` (facebook_conversions_service.py:375–381), а utm_session=NULL у 100% заказов → все CAPI-события уходят без клик-ID. Разбор: `audit_report_section2_analytics.md`. Где: utm_middleware.py (platform_data), Order (полей fbclid НЕТ в orders/models.py — только utm_*!), facebook_conversions_service.py. Что: клик-ID живут только в UTMSession/сессии; при COD-заказе (0 привязок к UTMSession) CAPI-события уходят БЕЗ fbc/fbp → низкий match quality. Задача: копировать fbclid/fbc/fbp/gclid/ttclid в Order (расширение TECH-060).
...
- [x] **AN-031. КРИТИЧНО: смена session_key при логине рвёт связку.** ПРОВЕРЕНО 05.07.2026 (код) — подтверждён на уровне кода: `login()` вызывает `cycle_key`, UTMSession ищется по новому session_key и не находится; сигнала миграции session_key нет в signals.py; visitor_id есть в модели (997/1015 заполнен) — готовый запасной ключ. Разбор и ТЗ: `audit_report_section2_analytics.md`. Где: utm_tracking.py (поиск UTMSession по `request.session.session_key`), Django auth. Что: воспроизвести: зайти с utm → залогиниться → оформить заказ; проверить, находит ли `link_order_to_utm` сессию. Если нет — мигрировать UTMSession.session_key при логине или искать по visitor_id.
...
- [x] **AN-035. Бот-фильтр фактически мёртв.** ПРОВЕРЕНО 05.07.2026 (код + БД) — подтверждён: 0/2899 is_bot; список бот-сигнатур узкий и is_bot-флаг фактически не проставляется на живом трафике; 36k product_view при 2.9k сессий указывает на бот-накрутку просмотров. Разбор и ТЗ (TECH-063): `audit_report_section2_analytics.md`. Где: `utm_utils.py::is_bot_user_agent`, БД: SiteSession is_bot=True = 0 из 2899. Что: либо ботов реально нет (маловероятно при 36k product_view), либо детект не работает/не пишет флаг; протестировать с UA «Googlebot»; расширить список; referrer-спам чёрный список (TECH-063).
...
- [x] **TD-033. CustomPrintLead воронка мертва.** ПРОВЕРЕНО 05.07.2026 (код + БД) — подтверждён: 28/28 new, 0 связок с заказами; эндпоинт смены статуса существует, но не используется; ось moderation_status работает независимо. Разбор и ТЗ (TECH-062): `audit_report_section3_techdebt.md`. Где: `storefront/models.py` (CustomPrintLead), БД: 28 лидов, все `new`. Что: статусная модель new→in_progress→quoted→won/lost + движение из админки (TECH-062); отчёт «лиды → выигранные».
...
| 05.07.2026 | CRO-041 | Аудит выполнен: разрыв подтверждён кодом (checkout.py не вызывает link_order_to_utm) и БД (0/41 utm_source, 41/41 COD); ТЗ TECH-060 детализировано | P0 done | audit_report_section1_cro.md |
| 05.07.2026 | CRO-042 | Аудит выполнен: 0/1015 converted; record_purchase/record_lead достижимы только из monobank-потока, онлайн-оплат не было; ТЗ TECH-061 | P0 done | audit_report_section1_cro.md |
| 05.07.2026 | AN-031 | Аудит выполнен: cycle_key при login() рвёт связку session_key→UTMSession; visitor_id заполнен у 997/1015 — рекомендован как запасной ключ поиска | P0 done | audit_report_section2_analytics.md |
| 05.07.2026 | AN-013 | Аудит выполнен: CAPI берёт fbc/fbp только из order.utm_session (NULL у 100% заказов) → события без клик-ID; fbclid в UTMSession есть у части сессий; расширение TECH-060 | P0 done | audit_report_section2_analytics.md |
| 05.07.2026 | AN-035 | Аудит выполнен: 0/2899 is_bot; расхождение 36k product_view vs 2.9k сессий — bot-накрутка вероятна; ТЗ TECH-063 | P1 done | audit_report_section2_analytics.md |
| 05.07.2026 | TD-033 | Аудит выполнен: 28/28 new, эндпоинт смены статуса есть но не используется, 0 лидов связано с заказами; ТЗ TECH-062 (quoted/won/lost + автопереходы) | P0 done | audit_report_section3_techdebt.md |
