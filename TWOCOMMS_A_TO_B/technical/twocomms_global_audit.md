...
- [x] **CB-040. Незапиненные зависимости.** ✅ Аудит 05.07.2026: боевые версии ��няты (openai==2.30.0, google-auth==2.52.0, google-analytics-data==0.22.0); пин — исполнителю → `audit_report_section6_codebase.md` Где: requirements.txt: `openai`, `google-auth`, `google-analytics-data` без `==`. Что: запинить текущие боевые версии (узнать через `pip freeze` на сервере в ОДНОЙ SSH-сессии); мажорный апдейт openai молча сломает AI-генерацию.
