...
- [x] **CRO-011. Пагинация каталога.** ✅ Аудит 05.07.2026: canonical/prev/next есть и корректны, дублей между страницами нет (order `-priority,-id`); **но пагинация СБРАСЫВАЕТ фильтры** (`href="?page=N"` затирает query string — P1); prev/next на фильтрованных URL указывают на нефильтрованные (P1-SEO); `/catalog/?page=N` дублирует showcase с self-canonical (P2-SEO) → `audit_report_section1_catalog.md` Где: `storefront/pagination.py`, catalog.py. Что: canonical/prev/next на страницах пагинации; переход по страницам не сбрасывает фильтры; нет дублирующихся товаров между страницами.
- [x] **CRO-012. Lazy-load изображений каталога.** ✅ Аудит 05.07.2026: lazy по умолчанию, width/height на 100% карточных img (CLS-safe), srcset+webp+preload hero — ОК; eager лишь у первых 2 карточек вместо 4–8 (первый ряд desktop = 4, P2) → `audit_report_section1_catalog.md` Где: catalog.html, `optimized_image.html`, `responsive_image.html`, `image_optimizer.py`. Что: `loading="lazy"` на всех карточках ниже фолда, первые 4–8 карточек eager; корректные width/height (нет CLS).
- [x] **CRO-013. Порядок сортировки: лонгсливы/худи первыми.** ✅ Аудит 05.07.2026 (частично): гриды сортируются единым `order_by(-priority,-id)` через админ-приоритет — кодом футболки не выпячены; **но showcase на `/catalog/` ставит tshirts выше hoodie** (конфиг `CATALOG_SHOWCASE_CARD_CONFIG`, P2, фикс-однострочник); сверка фактических priority в БД отложена до SSH-окна → `audit_report_section1_catalog.md` Где: catalog.py (queryset ordering), главная витрина. Что: дефолтная сортировка не выпячивает футболки вопреки позиционированию.
...
- [ ] **CRO-021. Тексты без «я», в tone of voice бренда.** Г��е: описания в БД (`Product.description*`, модели переводов django-modeltranslation: name_uk/ru/en), product_detail.html. Что: grep по описаниям на «я », «мне», «мой» от первого лица владельца; спорт/бейсбол-лексика; несоответствие концепции «difficulties».
...
- [ ] **AN-035. Бот-фильтр фактически мёртв.** Где: `utm_utils.py::is_bot_user_agent`, БД: SiteSession is_bot=True = 0 из 2899. Что: либо ботов реально нет (маловероятно при 36k product_view), либо детект не работает/��е пишет флаг; протестировать с UA ��Googlebot»; расширить список; referrer-спам чёрный список (TECH-063).
...
- [ ] **DB-005. N+1 в админ-отчётах.** Где: `services/admin_analytics.py`, `utm_cohort_analysis.py`. Что: prefetch/annotate вместо цикло�� по объектам; время загрузки вкладки аналитики < 3s.
...
- [ ] **CB-033. 16 inline-script в base.html.** Где: base.html (1403 строки). Чт��: инвентаризация каждого блока: что делает, можно ли вынести в файл (кэшируется) или удалить; особо — самописный GTM-загрузчик (см. CRO-004/AN-002).
...
| 05.07.2026 | CB-044 | Аудит crontab: 7 задач, вс�� скрипты в репо; НЕТ бэкап-cron, НЕТ feed-cron; логи в logs/ и tmp/ | P0 done | audit_report_section6_codebase.md |
