...
- [x] **CRO-023. Размерная сетка с карточки.** ✅ Аудит 05.07.2026: UI-каркас гайда на PDP есть (таб+2 ссылки), НО **30/65 published PDP без см-таблицы** (все 18 лонгсливов + 7 футболок + 5 худи имеют `catalog=None` → fallback без замеров); пресета longsleeve не существует; `view_size_guide` подтверждённо отсутствует (ACTION_TYPES/JS); побочно P1: SEO-блок best_prices линкует `/product/<id>/` → 8×404 на /catalog/long-sleeve/ (миграция 0053, rewrite не реализован) → `audit_report_section1_product.md` Где: `storefront/services/size_guides.py`, product_detail.html. Что: таблица замеров в см доступна с каждой карточки лонгслива/худи; событие `view_size_guide` (TECH-005) пока НЕ существует — создать задачу на добавление.
...
<<<<<<< HEAD
- [ ] **CRO-025. Выбор цвета/размера — UX и трекинг.** Где: product_detail.html JS, `productcolors/`. Что: смена цвета обновляет фото и це����у без перезагрузки; события `select_size`/`select_color` (TECH-008) отсутствуют — задача на добавление.
...
>>>>>>> origin/v0/as8gak6o36-2787-cddd3331
...
- [x] **CRO-041. КРИТИЧНО: COD-заказ не пишет UTM.** ✅ Аудит 05.07.2026: подтверждено до строк кода, +нюансы (session_key тоже пуст; monobank-исключения) → `audit_report_section1_cro.md` Где: checkout.py — в нём НЕТ вызовов `link_order_to_utm`/`record_purchase`/`record_lead` (они есть только в `storefront/views/monobank.py:565,584,986`). Подтверждено БД: 0 из 41 заказов имеют utm_source. Что: задача = вызывать `link_order_to_utm(request, order)` + `record_order_action` при создании ЛЮБОГО заказа (COD и предоплата). Это TECH-060.
- [x] **CRO-042. is_converted никогда не проставляется.** ✅ Аудит 05.07.2026: подтверждено; +самостоятельный дефект (webhook-контекст record_lead без сессии покупателя) → `audit_report_section1_cro.md` Где: `storefront/utm_tracking.py::record_lead/record_purchase` вызываются только из monobank-потока; БД: 1015 UTM-сессий, 0 converted. Что: после фикса CRO-041 проверить, что `mark_as_converted` срабатывает и для COD. Это TECH-061.
...
=======
- [x] **AN-030. UTM переживает всю воронку.** ✅ Аудит 05.07.2026: utm_data в Django-сессии переживает cycle_key, но UTMSession-строка — НЕТ (см. AN-031); SESSION_COOKIE_AGE — допроверить при SSH-окне → `audit_report_section2_analytics.md` Где: `storefront/utm_middleware.py` (session['utm_data']). Что: UTM из первого URL доступна на чекауте после 30+ минут и переходов; проверить SESSION_COOKIE_AGE и не рвётся ли session_key при логине (`django.contrib.auth.login` меняет session_key — UTMSession привязана к старому ключу! Проверить `cycle_key` эффект).
- [x] **AN-031. КРИТИЧНО: смена session_key при логине рвёт связку.** ✅ Аудит 05.07.2026: механизм доказан по коду, 6 точек логина; +риск дублей UTMSession на visitor_id → `audit_report_section2_analytics.md` Где: utm_tracking.py (поиск UTMSession по `request.session.session_key`), Django auth. Что: воспроизвести: зайти с utm → залогиниться → оформить заказ; проверить, находит ли `link_order_to_utm` сессию. Если нет — мигрировать UTMSession.session_key при логине или искать по visitor_id.
- [ ] **AN-032. Нормализация utm_source.** Где: живая БД: 'ig' (198), 'Instagram' (128), 'IGShopping' (6), 'Inst_Vid' (10) — 4 написания одного канала; 'fb'/'fb-SiteLink'; '120233970682840302' (сырой ad id). Что: словарь нормализации при записи или на уровне отчётов; UTM governance-конвенция (TECH-009).
- [ ] **AN-033. AI-источники как отдельный канал.** Где: БД: utm_source='chatgpt.com' 109 сессий + referrer chatgpt.com 18. Что: детект chatgpt.com/perplexity.ai/gemini/claude.ai по utm и referrer → канал «AI» в отчётах (TECH-065).
- [ ] **AN-034. Кэш не сбрасывает UTM.** Где: cache_headers.py, whitenoise, hosting-кэш (Hostsila/LiteSpeed?). Что: страница с `?utm_...` не отдаётся из кэша без прохода через UTMTrackingMiddleware; проверить, что HTML с query-параметрами — MISS или что мидлварь стоит до кэш-слоя.
- [x] **AN-035. Бот-фильтр фактически мёртв.** ✅ Аудит 05.07.2026: 0 ботов = ранний return ДО создания SiteSession (мёртвый код is_bot=bot); реальная дыра — UserAction без бот-фильтра + 2 разных детекта → `audit_report_section2_analytics.md` Где: `utm_utils.py::is_bot_user_agent`, БД: SiteSession is_bot=True = 0 из 2899. Что: либо ботов реально нет (маловероятно при 36k product_view), либо детект не работает/не пишет флаг; протестировать с UA «Googlebot»; расширить список; referrer-спам чёрный список (TECH-063).
- [x] **AN-036. increment_visit на каждый запрос без UTM.** ✅ Аудит 05.07.2026: уточнено — доминирует пустой SELECT на каждый pageview не-UTM трафика; UTM-визитёры инкремент почти не получают (elif-ветка) → `audit_report_section2_analytics.md` Где: utm_middleware.py (ветка `else: utm_session.increment_visit()`). Что: КАЖДЫЙ запрос любой страницы делает SELECT+UPDATE UTMSession — нагрузка и искажение visit_count (это pageviews, не визиты); оценить и переработать.
>>>>>>> origin/v0/as8gak6o36-2787-cddd3331
...
=======
- [ ] **CB-011. scripts/ внутр�� twocomms.** Где: `twocomms/scripts/fill_translations.py` (**3706 строк** — 2-й по величине файл проекта!), `wrap_themes_lazy.py`. Что: это одноразовые фазовые скрипты (Phase 17) — проверить, завершена ли фаза; если да — в архив.
>>>>>>> origin/v0/as8gak6o36-2787-cddd3331
...
=======
- [x] **CB-040. Незапиненные зависимости.** ✅ Аудит 05.07.2026: боевые версии снят�� (openai==2.30.0, google-auth==2.52.0, google-analytics-data==0.22.0); пин — исполнителю → `audit_report_section6_codebase.md` Где: requirements.txt: `openai`, `google-auth`, `google-analytics-data` без `==`. Что: запинить текущие боевые версии (узнать через `pip freeze` на сервере в ОДНОЙ SSH-сессии); мажорный апдейт openai молча сломает AI-генерацию.
>>>>>>> origin/v0/as8gak6o36-2787-cddd3331
...
=======
| RISK-14 | «Оптимизация» дублей img/ vs images/ бьёт по кэшированным URL в GMC-фиде/пиксел��х | CB-013 | Средняя / Средний | Сначала карта внешних потребителей URL (фиды, og:image), редиректы со старых путей |
>>>>>>> origin/v0/as8gak6o36-2787-cddd3331
...
| 05.07.2026 | CRO-023 | UI-каркас size guide на PDP полный (таб + 2 ссылки на /rozmirna-sitka/), НО **P1: 30/65 published PDP без см-таблицы** — все 18 лонгсливов + 7 футболок + 5 худи имеют catalog=None (SizeGrid недостижим, fallback без замеров); пресета longsleeve нет ни в коде, ни на /rozmirna-sitka/ (тупик для основного продукта); `view_size_guide` подтверждённо отсутствует (26 ACTION_TYPES без него, JS-таб молчит) — TECH-005 актуальна; P2: пресет-таблица недостижима без SizeGrid (source `preset_detected` — мёртвый код); ПОБОЧНО P1: SEO-блок best_prices (миграция 0053) хранит `/product/<id>/` — 8×404 битых ссылок на /catalog/long-sleeve/, обещанный rewrite не реализован (category_seo_blocks.html:42 выводит item.url как есть) | P1 done | audit_report_section1_product.md |
=======
| 05.07.2026 | CRO-041 | Подтверждено: create_order без UTM-вызовов и без session_key; monobank-исключения при падении invoice | P0 done | audit_report_section1_cro.md |
| 05.07.2026 | CRO-042 | Подтверждено + record_lead в webhook-контексте ищет сессию покупателя, которой там нет | P0 done | audit_report_section1_cro.md |
| 05.07.2026 | AN-013 | Order без полей клик-ID; CAPI берёт fbc/fbp только из payment_payload.tracking (пишет только monobank); **новый дефект: fbclid без utm_* теряется полностью** | P0 done | audit_report_section2_analytics.md |
| 05.07.2026 | AN-030/031 | cycle_key при login рвёт UTMSession-связку (6 точек логина); риск дублей UTMSession по visitor_id | P0 done | audit_report_section2_analytics.md |
| 05.07.2026 | AN-035 | 0 ботов = ранний return до создания SiteSession; UserAction вообще без бот-фильтра (объясняет 36k product_view); 2 несогласованных детекта | P1 done | audit_report_section2_analytics.md |
| 05.07.2026 | AN-036 | Пустой SELECT UTMSession на каждый pageview не-UTM трафика; visit_count семантически битый | P1 done | audit_report_section2_analytics.md |
| 05.07.2026 | INFRA | SSH к серверу начал сбрасываться на kex после первой сессии (anti-bruteforce?); повторная DB-сверка отложена, скрипт сверки описан в audit_report_section2_analytics.md | note | — |
>>>>>>> origin/v0/as8gak6o36-2787-cddd3331
