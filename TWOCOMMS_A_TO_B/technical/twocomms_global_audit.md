...
- [x] **CRO-024. Событие product_view — завышение.** ✅ Аудит 05.07.2026: **завышение ~27x подтверждено** — 34 944 из 36 263 (96,4%) product_view — «сироты» без SiteSession (боты, отсечённые middleware, но записанные record_user_action, где НЕТ бот-фильтра); честная человеческая база ≈ 1319 (PageView /product/ = 1000), реальный CTR view→cart ≈ 3,3%; (в) сервер+JS дубля нет, но есть серверный дубль на legacy `?size=` URL (record строка 247 до 301-редиректа 319–330); побочно P1: MySQL без tz-таблиц (CONVERT_TZ=NULL → все Trunc*-агрегации аналитики ломаются) → `audit_report_section1_tracking.md` Где: `storefront/tracking.py`, `storefront/utm_tracking.py::record_product_view`, вызовы в product.py. Что: 36 009 product_view против 44 add_to_cart — проверить: (а) пишется ли view при каждом AJAX/прелоаде/боте; (б) исключаются ли краулеры (`is_bot_user_agent`); (в) нет ли двойного вызова (сервер + JS).
...
| 05.07.2026 | CRO-024 | **Завышение product_view ~27x подтверждено:** 34 944/36 263 (96,4%) — сироты без SiteSession (боты; record_user_action не имеет бот-фильтра, is_bot_user_agent проверяется только в utm_middleware); человеческая база = 1319 (сверено с PageView=1000), честный CTR view→cart ≈ 3,3%; серверный дубль на legacy `?size=` URL (record до 301); два рассинхронизированных бот-детектора (utm_utils 12 паттернов vs tracking.BOT_SIGNALS 30); бот-хиты форсят django_session.save(); page_view-действий 0 (record_page_view не вызывается); product_view с auth user = 0/36263; **P1 побочно: MySQL без tz-таблиц — CONVERT_TZ=NULL, все TruncDate/ExtractHour аналитики возвращают None** | P0 done | audit_report_section1_tracking.md |
