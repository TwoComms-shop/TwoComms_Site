...
- [x] **CRO-023. Размерная сетка с карточки.** ✅ Аудит 05.07.2026: **P1 — у ВСЕХ 19 лонгсливов (основной продукт) сетки с замерами НЕТ** (пресета longsleeve не существует, size_grid/catalog = 0/0; на /rozmirna-sitka/ лонгсливов тоже нет); худи/футболки с catalog — таблица OK; `view_size_guide` не существует (TECH-005 подтверждена); **побочный P1: seo-pricing блок — 24 битые ссылки `/product/{id}/` → 404 на 3 категориях** (шаблон рендерит сырой item.url вместо get_absolute_url) → `audit_report_section1_product.md` Где: `storefront/services/size_guides.py`, product_detail.html. Что: таблица замеров в см доступна с каждой карточки лонгслива/худи; событие `view_size_guide` (TECH-005) пока НЕ существует — создать задачу на добавление.
- [x] **CRO-024. Событие product_view — завышение.** ✅ Аудит 05.07.2026: 3 root cause найдены в коде. **P1-1: `record_user_action` (utm_tracking.py) — ЕДИНСТВЕННЫЙ слой трекинга без bot-фильтра** (UTM-middleware и PageView боты отсекают, UserAction — нет; каждый бот-хит = строка product_view + НОВАЯ django_session); **P1-2: двойной счёт на 301** (record_product_view в product.py:247 ДО редирект-ветки :319–330); P2: нет дедупа перезагрузок/цветовых path-URL. Сервер+JS двойного счёта НЕТ (клиентский пуш отсутствует); prefetch не триггерит. Live-DB срез отложен (SSH connection reset, fail2ban) → `audit_report_section1_product.md` Где: `storefront/tracking.py`, `storefront/utm_tracking.py::record_product_view`, вызовы в product.py. Что: 36 009 product_view против 44 add_to_cart — проверить: (а) пишется ли view при каждом AJAX/прелоаде/боте; (б) исключаются ли краулеры (`is_bot_user_agent`); (в) нет ли двойного вызова (сервер + JS).
- [x] **CRO-025. Выбор цвета/размера — UX и трекинг.** ✅ Аудит 05.07.2026: фото/URL/canonical при смене цвета — образцово (Phase 8 replaceState); НО **цена НЕ обновляется при смене цвета** — `price_override` варианта уже используется в фидах (marketplace_feeds:782), а PDP-JS и корзина (cart.py:560 «Цена всегда берется из Product», 9 мест) его игнорируют → риск расхождения фид↔сайт; **размеры/цвета не блокируются по stock** (поле есть, в шаблоне/cart.py не используется → отменённые заказы); TECH-008 подтверждена: `select_size`/`select_color` = 0 вхождений, только Meta CustomizeProduct (GA4-воронка вариантов невозможна); a11y свотчей — нет aria-checked → `audit_report_section1_product.md` Где: product_detail.html JS, `productcolors/`. Что: смена цвета обновляет фото и цену без перезагрузки; события `select_size`/`select_color` (TECH-008) отсутствуют — задача на добавление.
...
- [ ] **TD-002. order_success_old.html.** Где: `twocomms_django_theme/templates/pages/order_success_old.html`. Что: grep по render/template_name — если не используется, удал��ть.
...
- [ ] **TD-033. CustomPrintLead воронка мертва.** Где: `storefront/models.py` (CustomPrintLead), БД: 28 лидов, все `new`. Что: статусная модель new→in_progress→quoted→won/lost + движение из админки (TECH-062); отчёт «лиды → выигран��ые».
...
- [ ] **CB-016. Пакеты-призраки в requirements.** Где: `requirements.txt` (81 строка). Что: для каждого пакета grep на импорт: кандидаты на неиспользуемые (django-ratelimit — TD-023 подтверждает 0 использований; google-analytics-data — AN-040). Удалять ��ольк�� после grep по ВСЕМУ коду включая scripts/.
...
- [x] **CB-040. Незапиненные зависимости.** ✅ Аудит 05.07.2026: боевые версии ��няты (openai==2.30.0, google-auth==2.52.0, google-analytics-data==0.22.0); пин — исполнит��лю → `audit_report_section6_codebase.md` Где: requirements.txt: `openai`, `google-auth`, `google-analytics-data` без `==`. Что: запинить текущие боевые версии (узнать через `pip freeze` на сервере в ОДНОЙ SSH-сессии); мажорный апдейт openai молча сломает AI-генерацию.
...
- [x] **CB-043. Git-состояние се��вера.** ✅ Аудит 05.07.2026 → `audit_report_section6_codebase.md` Где: сервер `~/TWC/TwoComms_Site` (в одной SSH-сессии: `git status`, `git stash list`, `git log -3`). Что: деплой = `git pull` — проверить, нет ли на сервере незакоммиченных правок (правки «на бою» будут конфликтовать с pull и молча теряться).
- [x] **CB-044. Crontab-инвентаризация.** ✅ Аудит 05.07.2026 → `audit_report_section6_codebase.md` Где: сервер `crontab -l` (одна SSH-сессия). Что: полный список задач → таблица «задача → команда → скрипт существует в репо? → лог»; выявить cron-зада��и, ссылающиеся на удалённые/переименованные скрипты (тихо падают).
...
| 05.07.2026 | CRO-006 | Мёртвого DOM на главной нет (един. display:none — легитимный load-more); survey-блок под флагом; рекомендация P2 — survey-с��кцию ниже товаров | P2 done | audit_report_section1_homepage.md |
...
| 05.07.2026 | CRO-011 | **P1-БАГ×2:** (1) пагинация `?page=N` затирает `?color=` — live-подтверждено; (2) `/catalog/?page=2..N` индексируемые (index,follow + self-canonical) дубли БЕЗ товаров — корень не рендерит г��ид, paginator строится зря; категорийная пагинация чиста (0 дублей между стр.) | P1 done | audit_report_section1_catalog.md |
...
| 05.07.2026 | CRO-025 | UX-ядро OK: смена цвета обновляет фото/миниатюры/offerId без перезагрузки, Phase 8 URL-sync (replaceState + canonical по Phase 7.3) образцовый, `?size=` legacy работает, Meta CustomizeProduct с очередью-стабом не теряет ранние клики; **находка 1: цена НЕ обновляется при смене цвета** — `ProductColorVariant.price_override` уже потребляется фидами (marketplace_feeds:782), но PDP-JS (0 вхождений updatePrice) и корзина (cart.py:560 «Цена всегда берется из Product», 9 мест) игнорируют → три поверхности могут отдавать разные цены (P1 если поле заполнено — live-check отложен, SSH reset); **находка 2: stock есть в модели, но размеры/цвета не дизейблятся и cart.py не валидирует** → отменённые заказы; **находка 3: TECH-008 подтверждена** — select_size/select_color = 0, GA4-воронка вариантов невозможна; P3: свотчи без aria-checked, мёртвый guard в trackCustomizeProduct | P2 done | audit_report_section1_product.md |
| 05.07.2026 | CRO-024 | **P1-1: `record_user_action` — единственный из 3 слоёв трекинга БЕЗ bot-фильтра** (utm_middleware:62 и tracking.py:35 ботов отсекают; UserAction пишет product_view на каждый хит googlebot/curl И создаёт новую django_session на каждый бот-хит — cookies бот не хранит); **P1-2: двойной счёт на 301** — record_product_view (product.py:247) вызывается ДО Phase 7.5 редирект-ветки (:319–330), legacy `?size=&color=` URL = 2 строки; P2: нет дедупа (F5/back/цветовые path-URL = отдельные строки); сервер+JS двойного счёта НЕТ, prefetch не триггерит; ratio 818:1 объясним: числитель «грязный» (боты), знаменатель «чистый» (боты не добавляют в корзину); 2 дублирующихся bot-детектора (is_bot 30 паттернов vs is_bot_user_agent 15) — рассинхрон; live-DB декомпозиция отложена (SSH reset ×4, fail2ban) | P1 done | audit_report_section1_product.md |
| 05.07.2026 | CRO-023 | **P1: у ВСЕХ 19 лонгсливов (основной продукт!) нет размерной таблицы** — пресета longsleeve не существует (в SIZE_GUIDE_PRESETS только hoodie/basic_tshirt), в БД SizeGrid=3 (hoodie, basic tee + тестовый артефакт «Test Catalog - Hoodie»), long-sleeve: size_grid 0/19, catalog 0/19 → глобальный фолбэк «звіряйтеся зі своєю річчю»; на /rozmirna-sitka/ лонгсливов тоже 0; +5 худи и 7 футболок без catalog тоже в фолбэке; view_size_guide не существует (grep=0, в ACTION_TYPES нет) — TECH-005 подтверждена; **побочный P1: seo-pricing «Найкращі ціни» — 24 битые ссылки `/product/{id}/`→404 на 3 категориях** (сид-миграция 0053:384 записала id-URL, обещанный rewrite не реализован — шаблон category_seo_blocks.html:42 рендерит сырой item.url вместо item.product.get_absolute_url; фикс = 1 строка) | P1 done | audit_report_section1_product.md |
