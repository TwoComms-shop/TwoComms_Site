...
| # | Задача | Приоритет |
|---|---|---|
| 1 | Перенест�� `record_product_view` после ветки 301-редиректа в product.py (двойной счёт) | P1 |
...
**Дополнение из отдельной sizeguide-ветки (слито сюда, отдельный `audit_report_section1_sizeguide.md` удалён как дубль workflow).** Ветка дополнительно зафиксировала полный скан 65 живых PDP: карточка size guide есть на 65/65, таблица замеров в см есть только на 35/65, fallback-заглушка без таблицы — на 30/65. Полный список fallback-товаров: `bentejne-ls`, `business-money-ls`, `death-gbs-ass-hd`, `death-gbs-ass-ls`, `death-gbs-ass-ts`, `death-grabs-ass-hd`, `death-grabs-ass-ls`, `death-grabs-ass`, `dvoznachni-summy-ls`, `hoodie-silent-winter`, `in-shee-ls`, `kha-edition-ls`, `kha-edition-ts`, `kha-style-ls`, `kharkiv-district-ls`, `last-breath-hd`, `last-breath-ls`, `longsleeve-classic`, `longsleeve-limited-edition`, `lord-of-the-lending-hd`, `lord-of-the-lending-ls`, `lord-of-the-lending`, `my-little-baby-ls`, `pojuy-ls`, `pokrovsk-girl-ls`, `red-leaves-ls`, `twocomms-beliveidea-ts`, `twocomms-reality-bends-dark-neon-edition`, `where-mi-present-ls`. Это подтверждает, что проблема не в отсутствии UI-блока, а в отсутствии таблицы/привязки данных.
...
## CRO-026. Блок отзывов на карточке (аудит 05.07.2026)

**Метод:** код `reviews/` (models/views/forms/urls/services/admin), partial `product_reviews.html` (238 строк), `seo_utils.py` (Product schema), view `product.py:552-573` + live-скан **всех 48 PDP** из sitemap-products.xml (curl, поиск `AggregateRating` и секции `id="product-reviews"`). Live-DB через SSH недоступен в этой сессии (host сбрасывает соединения после скана — kex reset; повторить позже).

### Что сделано ХОРОШО (инфраструктура образцовая)

- **Полный цикл в коде есть:** модель Review (rating 1-5, статусы pending/approved/rejected, `is_verified_purchase`, helpful/unhelpful счётчики), ReviewImage, ReviewVote, модерация в Django-admin (`approve_selected`), анти-спам форма, поддержка гостевых отзывов.
- **Секция отзывов рендерится на 48/48 живых PDP** (гистограмма, сводный рейтинг, форма отправки, идентити-блок «ваш статус»).
- **SEO-контракт правильный:** `aggregateRating` + вложенные Review JSON-LD эмитятся только из approved-отзывов через единый сервис `reviews/services/aggregate.py`; фейковых рейтингов нет. Порог осознанно снижен с 3 до 1 (`MIN_APPROVED_REVIEWS_FOR_RATING = 1`, SEO v1.0 Phase 12).
- **Запросы оптимальны:** summary — один aggregate-запрос, список — top-10 c `select_related("user")` + `prefetch_related("images")`, N+1 нет.

### НАХОДКА 1 (P1, бизнес, НЕ код): отзывов на сайте НОЛЬ — вся инфраструктура работает вхолостую

Live-скан всех 48 PDP: **0 страниц с `AggregateRating`**. Поскольку порог = 1 approved-отзыв, это означает: **ни один товар не имеет ни одного опубликованного отзыва**. Комментарий в коде сам это предсказал: «PDP traffic is too thin… extreme cold-start penalty» — но и после снижения порога до 1 отзывы не появились, т.к. **петля сбора отзывов не реализована**: упомянутый в коде «companion review-collection loop (Phase 13.x coupon → review)» — grep по репо = 0 вхождений реализации; после доставки заказа никакого email/TG-запроса «оставьте отзыв» не отправляется (в `orders/` и Telegram-уведомлениях нет ни одного упоминания reviews). Итог: SERP-звёзды (5-15% CTR-uplift, ради которого снижали порог) не получены ни на одном товаре. **Решение:** реализовать триггер после смены статуса заказа на «доставлен/выполнен»: письмо или TG-сообщение со ссылкой на PDP#product-reviews (+ мотиватор-промокод) — это единственный недостающий кусок, весь остальной конвейер готов.

### НАХОДКА 2 (P2): кнопок «Корисно/Не корисно» НЕТ — endpoint-сирота

`reviews/urls.py` содержит `vote/<id>/` → `vote_review` (POST, ReviewVote, пересчёт helpful_count), но **в шаблоне нет ни одной кнопки голосования** и во всём JS нет ни одного вызова `/reviews/vote/` (grep по templates+static = 0). Шаблон только *отображает* `helpful_count`, если он ненулевой (строка 228), а ненулевым он стать не может. Мёртвый функционал + сортировка `-helpful_count` в выдаче отзывов не имеет входных данных. **Решение:** добавить две кнопки + fetch-вызов (~20 строк) или удалить endpoint.

### НАХОДКА 3 (P3): рассинхрон копирайта с порогом рейтинга

Порог снижен до 1, но UI-тексты остались от порога 3: `product_reviews.html:62` — «Після 3 перевірених публікацій», комментарии в `product.py:554` («only at >=3 approved reviews») и docstring `aggregate.py` («only at ≥3») противоречат фактическому `MIN_APPROVED_REVIEWS_FOR_RATING = 1`. Покупатель с 1-2 отзывами увидит «Рейтинг скоро… після 3», хотя рейтинг уже показывается. Поправить текст и комментарии.

### НАХОДКА 4 (P3): отправка формы = full page reload без якоря

Форма отзыва — обычный `<form method="post">` без AJAX; после сабмита пользователь попадает наверх страницы, сообщение об успехе/ошибке нужно искать скроллом до секции. Мелочь, но снижает completion rate первой (критически дефицитной) волны отзывов. Решение: redirect с `#product-reviews` или AJAX-сабмит.

### Задачи исполнителю (из CRO-026)

| # | Задача | Приоритет |
|---|---|---|
| 1 | Петля сбора отзывов: триггер «заказ доставлен → email/TG со ссылкой на #product-reviews (+промокод)»; без неё вся review-инфраструктура и SERP-звёзды мертвы | **P1** |
| 2 | Кнопки «Корисно/Не корисно» + fetch на существующий `/reviews/vote/<id>/` (или удалить endpoint) | P2 |
| 3 | Синхронизировать тексты/комментарии с порогом 1 (шаблон:62, product.py:554, aggregate.py docstring) | P3 |
| 4 | Redirect на `#product-reviews` после POST формы отзыва | P3 |
| 5 | После восстановления SSH: проверить `Review.objects.count()` по статусам — есть ли зависшие pending-отзывы, которые никто не модерирует | P3 |

---

## CRO-027. Рекомендации на карточке (аудит 05.07.2026)

**Метод:** `storefront/recommendations.py` (231 строка, полное чтение), `product.py:462-478`, блок `tc-related-panel` в product_detail.html:677-720, fragment-cache анализ.

### Что сделано ХОРОШО

- **N+1 НЕТ (главный вопрос чек-листа — закрыт):** после получения рекомендаций view делает `prefetch_related_objects(recommended_products, 'color_variants__images')` bulk-ом + `build_color_preview_map`; `display_image` в шаблоне использует `_prefetched_objects_cache` (models.py:1041) — рендер 8 карточек без запросов на карточку. Плюс весь блок обёрнут в fragment-cache 3600с с ключом от `recommended_product_ids` + версии сортировки.
- **Каскад источников разумный:** та же категория → «часто покупают вместе» (по реальным OrderItem) → популярные; для главной — избранное/история пользователя.

### НАХОДКА 1 (P2): out-of-stock НЕ фильтруется — но это системная проблема, не локальная

Ответ на вопрос чек-листа: **да, движок может рекомендовать распроданный товар** — единственный фильтр везде `status=PUBLISHED`, поле `ProductColorVariant.stock` не проверяется ни в одном из 5 методов подбора. НО: как установлено в CRO-025 (НАХОДКА 2), stock не проверяется ВООБЩЕ нигде на витрине (PDP, корзина). Чинить только в рекомендациях бессмысленно — товар остаётся «покупаемым» на своей же PDP. Включить в общую задачу stock-enforcement (CRO-025 задача 2): когда появится правило «товар без остатков = недоступен», добавить его же в `_get_product_recommendations`/`_get_popular_products`.

### НАХОДКА 2 (P2): дубликаты в выдаче — товар может показаться дважды

`_get_product_recommendations` конкатенирует три списка через `extend` **без дедупликации**: товар из той же категории, попавший и в «часто покупают вместе» (или в популярные), появится в `recommendations` дважды → на PDP две одинаковые карточки рядом. Вероятность высокая: каталог маленький (48 товаров), категорийные и популярные множества сильно пересекаются. Также условие `if len(recommendations) < limit` перед «частыми» делает добор, но дубли съедают слоты у уникальных товаров. **Решение:** дедуп по id (`seen = {product.id}` + проверка перед extend) — 5 строк.

### НАХОДКА 3 (P3): бесполезная per-user фрагментация кэша движка

Ключ кэша `recommendations_{user.id|anon}_{product.id}_...` включает user id, но `_get_product_recommendations` (PDP-ветка) **вообще не использует self.user** — персонализация есть только в home-ветке. Итог: для N залогиненных пользователей × 48 товаров хранится N×48 идентичных копий списка (TTL 300с). Плюс несогласованность TTL: движок 300с, фрагмент шаблона 3600с — фактически правит фрагмент, движок почти всегда пересчитывается впустую для новых юзеров. **Решение:** для product-ветки ключ без user id; TTL выровнять.

### НАХОДКА 4 (P3): `_get_frequently_bought_together` учитывает отменённые заказы

Запрос по OrderItem не фильтрует статус заказа — отменённые/невыкупленные заказы участвуют в «часто покупают вместе» и в `_get_popular_products` (`Count('orderitem')`). На малых объёмах пара отменённых заказов заметно искажает ранжирование. Решение: `order__status__in=[оплаченные/выполненные]`.

### Задачи исполнителю (из CRO-027)

| # | Задача | Приоритет |
|---|---|---|
| 1 | Дедупликация рекомендаций по product.id (5 строк в `_get_product_recommendations`) | P2 |
| 2 | Stock-фильтр в рекомендациях — в составе общей задачи stock-enforcement из CRO-025 | P2 (связана) |
| 3 | Убрать user id из ключа кэша product-ветки; выровнять TTL движка (300с) и фрагмента (3600с) | P3 |
| 4 | Фильтр статуса заказа в `_get_frequently_bought_together` и `_get_popular_products` | P3 |
