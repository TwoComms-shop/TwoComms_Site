...
2. Точечный django-ratelimit: ТОЛЬКО 2 эндпоинта lookup в cart.py (:1752, :1789) и оба с `block=False` (��е блокирует, только помечает). **Логин, регистрация, чекаут, отзывы, /api/ — без индивидуальных лимитов** (login brute-force ограничен только глобальной soтней в минуту).
...
## TD-024. DRF API поверхнос��ь (АУДИТ ВЫПОЛНЕН, 06.07.2026)
