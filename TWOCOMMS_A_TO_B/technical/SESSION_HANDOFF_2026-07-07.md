...
## 6.1. ОБНОВЛЕНИЕ (сессия v0, ветка v0/pewemo5000-2082-f0c328cb, PR #44 + текущий коммит)

**ЗАКРЫТО в чеклисте (аудит-only, кода не трогали):**
- CB-001, CB-005, CB-006, CB-007 — PR #44 смерджен в main (07.07.2026).
- CB-002, CB-003 — текущий коммит. Все детали в `audit_report_section6_codebase.md`.

**⚠️ P0-НАХОДКА (передать владельцу НЕМЕДЛЕННО):**
- `deploy_finance.sh` (tracked, строка 5) содержит НЕзамаскированный SSH-пароль прода
  (`export SSHPASS='...'`). Это единственный оставшийся файл — прошлая зачистка
  (плейсхолдеры `***REMOVED_SSH_PASSWORD***` в .exp/deploy_*.sh) его пропустила.
  Действия: сменить пароль qlknpodo / перейти на ключи → замаскировать строку → коммит.
  Историю git НЕ переписывать без согласования.

**Другие важные находки этой сессии:**
- `twocomms/storefront/views.py.backup` — ЖИВОЙ рантайм-код (грузится через
  `_load_legacy_views()` в `storefront/views/__init__.py:329`, обслуживает
  `/pricelist_opt.xlsx` и др.). НЕ удалять при чистке backup-файлов!
- Личное фото владельца `me.JPG` (4.8MB) tracked в 2 копиях, вкл.
  `twocomms/static/BrandDNA/` (попадает в collectstatic).
- SSH из sandbox v0 по-прежнему блокируется (kex reset) — CRO-051 всё ещё ждёт
  сессию с рабочим SSH.

**Осталось открытых пунктов:** см. `grep -c '^- \[ \]' twocomms_global_audit.md`
(было 35 до этой сессии, стало 29).
...
2. ��формить разделы отчёта по AN-011 / AN-012 на основе файлов из п.4–5.
