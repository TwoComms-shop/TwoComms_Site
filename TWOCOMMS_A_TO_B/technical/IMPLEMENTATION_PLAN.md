...
- [x] **W2-1. 🔴 Единая UTM-привязка любого заказа (CRO-041 / AN-013 / AN-021 / AN-030 / AN-031 → TECH-060)** `[REPO]` ✅ fallback-цепочка session_key→visitor_id→session['utm_data'] в link_order_to_utm; attach_tracking_to_order пишет click-ID (fbp/fbc-синтез из fbclid/ttclid/gclid/external_id/ip/ua) в payment_payload.tracking для COD; тесты test_utm_attribution.py (3) зелёные. Единый order-builder COD+Monobank — отложен как долгосрочный рефакторинг.
  ...
  > **RE-CLOSED 2026-07-16:** COD and guest-prepay linkage acceptance was already proven by `394a247c` + `30808819`; the last CheckoutCapture residual was closed by `de7f7efc` + `1962b488`, server 137/137, MariaDB canary and 4/4 historical reconciliation.
  **Historical snapshot:** COD-путь не вызывал функции трекинга; даже в monobank `link_order_to_utm` не срабатывал из-за lookup строго по session_key и `cycle_key()` при логине. Click-ID не доходили до CAPI для COD; baseline был 0/43 заказов с utm.
...
- [x] **W3-11. [NEW-510] CheckoutCapture: публичный PII-приёмник без лимитов и retention (P2)** `[REPO]`
  > **RE-VERIFY W3-11:** RESOLVED 2026-07-16; invalid payloads fail before mutation, COD/Mono terminal transitions are race-safe, and production historical matches were reconciled.
  **Historical snapshot:** `checkout_capture.py` писал ФИО/телефон/email по session_key с одной Sec-Fetch-Site защитой, без rate-limit и retention; этот baseline закрыт пунктами DONE ниже.
  Фикс: rate-limit по session/IP; чи��тка capture-записей старше 30-90 дней в trim-команде; упомянуть в privacy policy.
  ✅ **DONE:** (1) rate-limit `@ratelimit(key='user_or_ip', rate='30/m', block=False)` + JSON 429 — 30/m хватает для дебаунс-автосейва формы, душит скриптовый спам; (2) retention: в `cleanup_analytics_data` добавлен шаг 4 — CheckoutCapture старше `--captures-days` (по умолчанию 60) удаляются, включая конвертированные (данные уже в Order); (3) упоминание в privacy policy — `[OWNER]` (связка с NEW-403); (4) `a90191ea..1962b488` требует actionable phone/email, сохраняет converted как terminal state и создаёт PII-free marker для успешных COD/Mono заказов. Production: InnoDB canary PASS, live invalid 6/6, count 10→10, четыре order-matched active rows reconciled до converted.
...
|---|---|---|---|
| 16.07.2026 | F-051 / F-075 / W2-1 / W3-11 | Checkout capture fail-closed validation, normalized phone/account-email policy, locked narrow upserts and race-safe PII-free terminal markers for COD/Mono. Local/server 137/137, MariaDB rollback canary, live negative 6/6 with count 10→10; historical captures 2/4/7/8 matched Orders and were atomically reconciled, leaving 6 legitimate active rows | a90191ea, 051db5a2, de7f7efc, 1962b488, c2945228 |
