...
5. **Итог:** COD-заказы (9 шт., без payment_payload) → CAPI-события уходят только с хешами phone/name/city → низкий Event Match Quality. Monobank-заказы пол��чают fbc/fbp только если cookie реально стояла в момент создания инвойса.
...
**Post-fix 16.07.2026 (F-010, `efd7f192`):** маршрут удалён из
production URL resolver вместе с шестью родственными debug/dev/test
маршрутами и сохраняется только при `DEBUG=True`. Server regression class
7/7; production `DEBUG=False`; cache-busted UK/RU/EN live matrix 21/21
вернула hard 404 без redirect, `/healthz/` остался 200. Риск загрязнения
боевых пикселей через `/test-analytics/` закрыт.
...
- если `win.gtag` уже создан (после загрузки gtag.js) → `gtag('event', 'AddToCart', payload)` — ПЛОС��ИЙ payload;
...
## AN-011. Meta CAPI — серверные события (АУДИТ ВЫПОЛНЕН, код-слой, 07.07.2026) — retry дублей не создаёт, но вся отправка живёт ВНУТРИ row-lock-транзакции; send_lead_event Meta — мёртвый API

**Карта реальных call-sites `facebook_conversions_service.py` (850 строк):**

| Метод | Call-sites | Когда стреляет |
|---|---|---|
| `send_purchase_event` | `storefront/views/utils.py:641` (monobank paid/prepaid/partial), `orders/nova_poshta_service.py:507` (НП delivered→done, COD offline-Purchase) | 2 живых пути |
| `send_add_payment_info_event` | `storefront/views/monobank.py:1002` (создание инвойса) | 1 живой путь |
| `send_lead_event` | **0 call-sites — МЁРТВЫЙ API** | никогда |
| `send_event_for_order_status` | 0 call-sites (уже зафиксировано в AN-014) | никогда |

**Retry-логика `_send_request_with_retry` (строки 283–313) — дублей НЕ создаёт:**
1. Настройки: `FACEBOOK_CAPI_MAX_RETRIES`=3, delay=1s, backoff=2 (settings-переменные, дефолты в коде).
2. Retry только на exception; **event_id детерминирован и НЕ перегенерируется между попытками** (`order.get_purchase_event_id()` = `{order_number}_{int(created.timestamp())}_{event_type}`, models.py:247–248) → даже если запрос дошёл до Meta, а ответ потерялся, повторная отправка с тем же event_id дедуплицируется на стороне Meta (окно 48ч). Требование чек-листа «retry не создаёт дублей» — выполнено by design.
3. Идемпотентность поверх retry: флаг `payment_payload.facebook_events.purchase_sent` проверяется ДО отправки в обоих путях (utils.py:637, nova_poshta_service.py:501) — повторный webhook/крон-тик не шлёт второй Purchase.

**Находки:**
1. **P1: весь блок отправки Meta+TikTok (utils.py:627–710) выполняется внутри `_record_monobank_status_locked`, который вызывается из `transaction.atomic()` + `Order.select_for_update()` (utils.py:479–485).** Худший случай: Meta 3 попытки с sleep 1+2s + TikTok timeout 15s ≈ до ~25–40s удержания row-lock заказа и открытой транзакции во время внешних HTTP-вызовов. Конкурентный webhook+return по тому же заказу блокируется; коннект БД занят. Плюс: lock попутно сериализует гонку двойной отправки. Фикс: вынести отправку событий за пределы транзакции (после commit, `transaction.on_commit`).
2. **P2: `send_lead_event` Meta никогда не вызывается** — клиентский fbq Lead (cart.js:790) уходит без серверной пары; внутренняя стата lead (record_lead) с Meta Lead не синхронизирована. Либо подключить (симметрично TikTok Lead при prepaid, utils.py:674), либо удалить как мёртвый код.
3. P3: `time.sleep` в retry — синхронная блокировка воркера (усиливает находку 1).
4. Токен/включённость: `FACEBOOK_CONVERSIONS_API_TOKEN`/`FACEBOOK_PIXEL_ID` из env (settings.py:1273/1277), при отсутствии — `enabled=False` с громким логом (fail-safe корректен).
5. **P3 (новое, 07.07.2026): fallback `event_source_url` указывает на ЧУЖОЙ домен `https://twocomms.com/orders/...`** — боевой домен `twocomms.shop` (settings.py ALLOWED_HOSTS:122–124). Встречается в 3 местах facebook_conversions_service.py (строки 561, 662, 779) и в tiktok_events_service.py:213. Все серверные события, где `source_url` не передан (а он НЕ передаётся ни в одном из живых call-sites: utils.py:641, nova_poshta_service.py:507, monobank.py:1002), уходят в Meta/TikTok с URL несуществующей страницы чужого домена. Для Meta `event_source_url` участвует в верификации домена и диагностике источника — событие с посторонним доменом может понижать доверие/атрибуцию. Фикс-однострочник: `https://twocomms.shop/...` или строить от `settings.SITE_URL`.

**Остаток (SSH/владелец):** grep боевых логов monobank_logger на `✅ Facebook Purchase event sent` / `❌` за последние 30 дней (реальная доставка + доля ошибок), валидность токена — тестовый event через Graph API Events Manager.

---

## AN-012. Дедупликация Pixel↔CAPI (АУДИТ ВЫПОЛНЕН, код-слой, 07.07.2026) — TECH-064 в коде РЕАЛЬНО СДЕЛАН для Purchase и AddPaymentInfo; Lead-пары не существует

**Проверка пары event_id по каждому событию:**

| Событие | Сервер (CAPI) | Клиент (fbq) | Пара? |
|---|---|---|---|
| Purchase | `order.get_purchase_event_id()` (utils.py:641→service:640) | `{{ order.get_purchase_event_id }}` в order_success.html:2130 (dataLayer) и `purchaseEventId`→`__meta.event_id`→fbq eventID (order_success.html:2183–2213) | **ДА — детерминированный, идентичный** |
| AddPaymentInfo | `get_add_payment_event_id()` (monobank.py:1002) | сервер возвращает `data.add_payment_event_id` в ответе инвойса, клиент использует его (checkout-mono.js:310/502: `data.add_payment_event_id \|\| safeGen()`) | **ДА, с оговоркой**: fallback на случайный id, если ответ без поля |
| Lead | НЕ отправляется (0 call-sites, AN-011) | fbq Lead стреляет (cart.js:790) | пары нет — дедуп не нужен, но серверного усиления нет |
| COD-Purchase (НП delivered) | отправляется кроном | клиент НЕ стреляет (gating `paymentStatus paid/prepaid/partial`, order_success.html:1889–1892; COD=unpaid) | коллизии нет by design |

**Свойства event_id (models.py:234–248):** `{order_number}_{int(created.timestamp())}_{event_type}` — детерминирован от создания заказа (не от момента вызова), стабилен при F5/повторном кроне/retry. Фоллбэк `int(time.time())` только если `created` пуст (практически недостижимо).

**Находки:**
1. TECH-064 (числился «сделан, не проверено») — **код-слоем ПОДТВЕРЖДЁН** для Purchase/AddPaymentInfo: одинаковый event_id в браузерном eventID и серверном событии.
2. P3: у AddPaymentInfo клиентский fallback `safeGenerateAnalyticsEventId()` при отсутствии `add_payment_event_id` в ответе → рассинхрон пары в этом edge-case (сервер уже отправил своё событие со своим id).
3. Наследованные ограничения силы дедупа: клиентский Purchase дедуплицируется sessionStorage-ключом (новый браузер/устройство → повторный fbq Purchase, Meta схлопнет по event_id — штатно); `META_PIXEL_CAPI_DEDUPE_IMPLEMENTATION.md` (корень) соответствует реальному коду.

**Остаток (владелец):** Events Manager → вкладка события Purchase → блок Deduplication (доля deduplicated browser/server) + зафиксировать EMQ в журнале. Кодом это не проверяется.

---

## AN-020. TikTok Pixel D43L7DBC77UA61AHLTVG (АУДИТ ВЫПОЛНЕН, код-слой, 07.07.2026) — **P1: и клиент, и сервер шлют НЕСТАНДАРТНЫЕ для TikTok имена «Purchase»/«Lead» вместо CompletePayment/SubmitForm**

**Клиент (analytics-loader.js):** ttq грузится отложенно с буферизацией (`_ttqLoaded`/`_ttqScriptLoaded`, буфер строк 1255–1314), `ttq.page()` после реальной загрузки скрипта (1199–1202), `ttq.identify` с хэшированными данными для залогиненных (558–614). Событие уходит как `win.ttq.track(eventName, ttqPayload)` (строка 394) — **имя события НЕ мапится**: в TikTok уходят Meta-имена ViewContent/AddToCart/InitiateCheckout/**Purchase**/Lead/Search/…; `ttqPayload.event_id` проставляется (382–384).

**Сервер (`orders/tiktok_events_service.py`, 308 строк):** legacy-endpoint `v1.3/pixel/track/`; `send_purchase_event` → `send_event(order, 'Purchase', order.get_purchase_event_id())` (строки 279–287), `send_lead_event` → `'Lead'` (289–297); user-контекст хэширован SHA-256 (email/phone/external_id/city), ttclid из `payment_payload.tracking` (разрыв COD = AN-021); retry НЕТ (одна попытка, timeout 15s, ответ `code==0` проверяется); test_event_code из env.

**Находки:**
1. **P1: «Purchase» и «Lead» НЕ входят в стандартный словарь TikTok** (стандарт: CompletePayment, PlaceAnOrder, SubmitForm, Contact…). Оба слоя шлют одинаковые нестандартные имена → события попадают в TikTok как кастомные: оптимизация кампаний на стандартную цель «Complete Payment» и стандартные воронковые отчёты их НЕ видят. Требование чек-листа «CompletePayment стреляет» — НЕ выполнено ни клиентом, ни сервером. Фикс: маппинг Purchase→CompletePayment, Lead→SubmitForm в analytics-loader (один словарь) и в tiktok_events_service (2 строки); event_id при этом сохраняет пару.
2. **Дедуп клиент↔сервер:** event_id идентичен (оба слоя используют `order.get_purchase_event_id()`/`get_lead_event_id()` — та же пара, что Meta) и имена совпадают («Purchase»==«Purchase») → дедуп технически возможен; НО paid-ветка сервера шлёт Purchase БЕЗ pre-check `purchase_sent` (utils.py:694, уже P2 в CRO-045) — повторный webhook = повторная отправка, спасает только дедуп TikTok по event_id.
3. P2: legacy-endpoint `open_api/v1.3/pixel/track/` — TikTok мигрирует на Events API 2.0 (`/event/track/`); работоспособность текущего — проверить живым тестовым событием.
4. P2: серверный TikTok Purchase уходит только при `payment_status=='paid'` (utils.py:690) — prepaid получает только Lead, COD не получает ничего (= AN-021/CRO-045, закрывается TECH-070).
5. P3: retry-логики нет вообще (контраст с Meta) — сетевые сбои теряют событие безвозвратно (флаг `tiktok_events.purchase_sent` при этом НЕ ставится, повторный webhook дошлёт — частичная самокоррекция).
...
**Архитектура (проводка реальна, не мёртвый код):**
- Вкладка «Диспетчер»: `/admin-panel/?section=dispatcher` → `admin.py::_build_dispatcher_context` (:2828) → `utm_analytics.py` (707 строк, 11 функций) + `utm_cohort_analysis.py` (474 строки: LTV/repeat-rate/когорты) → `partials/admin_dispatcher_section.html`.
- DRF API: `/api/utm/*` — `UTMAnalyticsViewSet` (18 actions: general_stats/sources/campaigns/funnel/geo/devices/roi/cohort_analysis/ab_test…), `permission_classes=[IsAdminUser]` — доступ закрыт корректно.
- CSV-экспорт: `/api/utm/export-csv/` под `@staff_member_required` — тоже закрыт.

**Выводы:**
1. **P1 (следствие CRO-041/042, самостоятельного бага нет): почти вся вкладка — системные нули.** Конверсии считаются от `is_converted` (0/1015 в БД), доход/AOV — от `Order.objects.filter(utm_session__in=…, payment_status='paid')` при 0/41 заказов с utm_session FK → conversions=0, revenue=0, ROI=−100%, LTV=0, когорты пустые, воронка обрывается после initiate_checkout. Достоверны только: sessions, unique_visitors, geo, devices/browsers, returning visitors. Требование чеклиста «цифры сходятся с БД» выполняется тривиально — и там, и там нули; после фикса TECH-060/061 перепроверить обязательно.
2. **P2: N+1 на каждый рендер вкладки.** `get_sources_stats`/`get_campaigns_stats`/`get_content_stats` в цикле по строкам делают по 2–3 доп. запроса (revenue + score на источник); при limit=20 это 40–60 запросов на блок, а `_build_dispatcher_context` собирает ~10 блоков за один GET → сотни запросов на открытие вкладки. Фикс: annotate/Subquery одним запросом на блок.
3. **P2: нет нормализации utm_source** — `ig` (198) и `Instagram` (128) считаются разными источниками во всех разрезах (подтверждено данными БД от 05.07). Маппинг-словарь при записи UTMSession решает.
4. **P3: unique_visitors считается по distinct IP** (`values('ip_address')`) — мобильный CGNAT/офисный NAT схлопывает разных людей, дубли Wi-Fi/LTE раздувают. При наличии `visitor_id` (пусть заполнен лишь у 128/1015) честнее считать по нему с fallback на IP.
5. **P3: revenue учитывает только `payment_status='paid'`** — доставленные COD-заказы (оплата при получении) в доход вкладки не попадут даже после фикса FK, если статус не переводится в paid при выдаче. Сверить с NP-кроном (AN-014).
6. **P3: ошибки глушатся в нулевой контекст** — `_build_dispatcher_context` при исключении возвращает нули + error; админ не отличит «нет данных» от «всё сломалось», если error не выводится заметно в шаблоне.
7. Экспорт CSV код-слоем существует и закрыт правами; фактическую выгрузку (кодировка/разделители/объём) проверить владельцем в браузере — кодом не верифицируется.

**Остаток (браузер/владелец):** открыть вкладку Диспетчер на бою, убедиться, что error-блок не активен; после фикса CRO-041/042 — повторная сверка цифр с прямыми SQL-запросами.
...
| 16.07.2026 | AN-015 / F-010 | `efd7f192`: семь test/debug/dev routes теперь только при DEBUG=True; server 7/7, production DEBUG=False, live UK/RU/EN 21/21 hard 404; `/healthz/` 200 |
...
| 07.07.2026 | AN-011 | Meta CAPI код-слой: retry дублей не создаёт (де��ерм. event_id + флаг purchase_sent); **P1 — отправка Meta+TikTok живёт внутри row-lock-транзакции (до ~25–40s удержания)**; send_lead_event Meta — мёртвый API (0 call-sites); P3 — fallback event_source_url на чужой домен twocomms.com (4 места); остаток — боевые логи и валидность токена по SSH |
| 07.07.2026 | AN-012 | Дедуп Pixel↔CAPI: TECH-064 код-слоем ПОДТВЕРЖДЁН для Purchase (идентичный get_purchase_event_id в fbq eventID и CAPI) и AddPaymentInfo (add_payment_event_id из ответа инвойса); Lead-пары не существует (серверный Lead Meta не шлётся); P3 — клиентский fallback random-id у AddPaymentInfo рвёт пару в edge-case; остаток — Deduplication/EMQ в Events Manager владельцем |
| 07.07.2026 | AN-020 | **P1: и клиент (ttq.track без маппинга имён), и сервер (tiktok_events_service) шлют нестандартные для TikTok имена «Purchase»/«Lead» вместо CompletePayment/SubmitForm — стандартные цели оптимизации их не видят**; P2 — legacy endpoint v1.3/pixel/track; P2 — server Purchase только при paid (prepaid=Lead, COD=ничего); P3 — retry нет; дедуп клиент↔сервер по event_id технически возможен; остаток — TikTok Events Manager и env по SSH |
| 07.07.2026 | AN-038 | UTM-вкладка «Диспетчер»: проводка реальна (admin.py:2828 + DRF /api/utm/ под IsAdminUser + CSV под staff_member_required); **P1 — почти все метрики системные нули (следствие CRO-041/042: is_converted=0, Order.utm_session FK=0)**; P2 — N+1 (сотни запросов на рендер вкладки); P2 — нет нормализации utm_source (ig/Instagram дубли); P3 — unique_visitors по IP, revenue только paid (COD выпадет), ошибки глушатся в нули; остаток — браузерная проверка вкладки + пересверка после TECH-060/061 |
