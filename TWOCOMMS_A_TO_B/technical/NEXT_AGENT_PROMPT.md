...
**Осталось 10 незакрытых пунктов** (сессия 07.07.2026 №3 закрыла SEO-006 и SEO-007 — полный краул 489/489 завершён, отчёты записаны):

| ID | Строка (~) | Суть | Что нужно |
|---|---|---|---|
| CRO-051 | 126 | Базовая линия конверсии из живой БД UserAction | SSH + Django shell |
| TD-015 | 193 | passenger_wsgi + лимиты воркеров/памяти | SSH (code-часть готова, см. §4.4) |
...
**ВСЕ 10 требуют SSH. SSH из песочницы НЕ РАБОТАЕТ** — подтверждено в третий раз (см. §3): порт 22 теперь ОТКРЫТ, но сервер сбрасывает соединение на kex-этапе (`kex_exchange_identification: Connection reset by peer`) → IP песочницы в блок-листе (fail2ban/хостинг-файрвол). Единственный путь — владелец запускает готовые скрипты на сервере (§5 п.2).
...
- Пароль не хранится в репозитории. Если владелец даст его в чате, использовать только для текущего SSH-вызова через `SSHPASS`/secret manager и НЕ коммитить в файлы, логи или отчёты.
...
1. ~~Краулер~~ ✅ ЗАВЕРШЁН (сессия №3): 489/489 + фазы 3–4, `{"type":"done"}` в JSONL. SEO-006/SEO-007 закрыты в чек-листе, разделы добавлены в `audit_report_section4_seo.md`, сводка — `data/seo_crawl_analysis_report.md`. Больше НЕ запускать краулер.
2. **Сразу скажи владельцу про SSH:** «SSH из песочницы недоступен (IP заблокирован на kex-этапе), запустите на сервере `scripts/server_shell_batch.sh` и `python manage.py shell < scripts/server_audit_batch.py`, пришлите вывод». Это закроет разом все 10 оставшихся пунктов.
3. **Code-часть TD-015 уже сделана (§4.4, P1-риск с мёртвой Celery-очередью задокументирован в чек-листе).** Предложи владельцу фикс `async_enabled=False`.
...
- Отчёт�� **на русском**, стиль — `audit_report_section4_seo.md` (раздел SEO-010 — образец).
